<template>
  ...
  <form @submit.prevent="">
    <label id="login-label">Login or Create </label>
    <div>
      <div style="float: left; width: 68%" id="input-div">
        <input
            autocomplete="off"
            id="input-id"
            name="newTodo"
      ...
      <div id="button-div" style="float: right; width: 30%; margin-right: 2%">
      ...
      <div id="one-button-div" style="float: right; width: 20%" hidden="true">
        <button v-on:click="reload()">Logout</button>
      </div>
    </div>
      ...
      axios.post("http://localhost:3000/create?username=" + document.getElementById('input-id').value)
          ...
          .then((response) => {
            if (response.status === 200) {
              ...
              document.getElementById('input-div').style.width = '80%';
              document.getElementById('input-id').disabled = true;
              document.getElementById('login-label').textContent = "Logged as";
